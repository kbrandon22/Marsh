...
global_fit_pooled <- pool_fitstats(global_fit)
    #c-hat = 1.76, Chi-square statistic = 0.001 indicating moderate overdispersion, 
    #visualize model fit with deviance plot

#5) Visualize observed vs simulated deviance
#a) Extract the bootstrapped and observed Chi-square statistics
boot <- sapply(global_fit, function(fit) fit@t.star[,2])
boot <- unlist(boot)
observed <- sapply(global_fit, function(fit) fit@t0[2])
